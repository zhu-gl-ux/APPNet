...
class BaseOptions:
    ...
    def __init__(self):
        ...
        parser.add_argument(
            "--resize_size", required=True, metavar="N", nargs="+", type=int, help="input size, e.g. 2048 512"
        )
        ...
        parser.add_argument(
            "--pretrained_aggrement",
            nargs="+",
            default=[""],
            type=str,
            help="pretrained weight (s) aggre module",
        )
        self.parser = parser
    ...
    def parse(self):
        ...
        if len(args.pretrained_aggrement) == 1:
            args.pretrained_aggrement = args.pretrained_aggrement[0]
