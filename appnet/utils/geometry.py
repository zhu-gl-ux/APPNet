...
def merge(patches, coords, output_size):
    ...
    _,B, C, _, _ = patches.shape

    output = torch.zeros((1,B, C, output_size[1], output_size[0]), device=patches.device, dtype=patches.dtype)
    ...
    patches = F.interpolate(patches, (C,ymax - ymin, xmax - xmin), mode="trilinear", align_corners=False)
    # print(patches.shape)
    ...
    output = output / torch.maximum(mask.unsqueeze(0).unsqueeze(0).unsqueeze(0), mask_ones.unsqueeze(0).unsqueeze(0).unsqueeze(0))
